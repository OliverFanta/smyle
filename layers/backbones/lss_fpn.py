...
class LSSFPN(nn.Module):
    ...
    def _forward_single_sweep(self,
        ...
        # Use original reshape logic
        source_features = img_feats[:, 0, ...]  # Get features for the first camera

        # Reshape according to original logic
        depth_feature = self._forward_depth_net(
            source_features.reshape(batch_size * num_cams,
                                    source_features.shape[2],
                                    source_features.shape[3]),  # Ensure correct dimensions
            mats_dict)

        depth_feature = depth_feature[:, :self.depth_channels].softmax(1)

        # If depth_oracle is provided, update depth predictions accordingly
        if depth_oracle is not None:
            b, c, h, w = depth_feature.shape
            fg_mask = (torch.max(depth_oracle, dim=1).values > 0.0).view(-1)

            depth_flattened = depth_feature.permute(0, 2, 3, 1).contiguous().view(-1, c)
            ...
            img_feat_with_depth = depth_updated.unsqueeze(1) * depth_feature[:, self.depth_channels:(self.depth_channels + self.output_channels)].unsqueeze(2)
        else:
            img_feat_with_depth = depth_feature.unsqueeze(1) * depth_feature[:, self.depth_channels:(self.depth_channels + self.output_channels)].unsqueeze(2)
        ...
        # Replace voxel pooling with an alternative method (e.g., averaging)
        feature_map = img_feat_with_depth.mean(dim=4)  # Example: average pooling
